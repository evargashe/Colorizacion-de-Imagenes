...
class TrainModel(BaseModel):
    ...
    def get_current_visuals(self):
        ...
        if self.opt.stage == 'full' or self.opt.stage == 'instance':
            ...
            self.instance_mask = torch.nn.functional.interpolate(torch.zeros([1, 1, 176, 176]),size=visual_ret['gray'].shape[2:], mode='bilinear', align_corners=False).type(torch.cuda.FloatTensor)
