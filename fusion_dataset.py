...
class Training_Instance_Dataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print("imprimir pred_bbox", pred_bbox)
        if pred_bbox.size == 0:  # Si pred_bbox está vacío
            raise ValueError(f"No bounding boxes found for image {self.IMAGE_ID_LIST[index]}")

        rgb_img, gray_img = gen_gray_color_pil(output_image_path)

        index_list = range(len(pred_bbox))
        index_list = sample(index_list, 1)  # Ahora sabemos que `pred_bbox` no está vacío
